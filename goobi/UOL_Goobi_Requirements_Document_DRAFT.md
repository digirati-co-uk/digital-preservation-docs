# UOL Goobi Requirements Document - DRAFT v3.1
13/05/2024
	...
	- b. By polling (requesting) digitisation tickets from the EMu requests module API. #Development 
		...
		- #done Leeds Goobi to poll Emu every 1 hour for request module
			...
			- #done Having a separate workflow for digitisation without cataloguing is necessary, but not just for images to be joined with catalogue records. The requirement may actually be two workflows: 
				...
				2. #done A workflow to capture images which will not be joined with records/IDs or added to stores/catalogue (e.g. digitisation of non-library content.) These images would bypass the `matching` step and go straight to the `sending` stage._
...
4. #done Leeds Access rights/sensitivity/licence and copyright information must be in place for an item/object and stored in the METS before it can be ingested into the digital preservation repository, at item/object level these will be pulled across from Emu and can be overwritten at a file level.
...
1. At the end of all processes the master files and the metadata will be delivered to the S3 environment for ingestion into the digital preservation repository. #Development 
...
2. After successful ingestion has been verified, and after a waiting period, the master files will be deleted from Goobi and only low resolution thumbnails will be kept for reference. #Development 
	- #intranda take the installation and configuration of multiple additional plugins into account in the pricing proposal.
3. If, after completion, a process needs to be edited then Goobi will pull the original master files from the digital preservation repository so that changes can be made (additions to files, deletion of files etc.) before reexporting the changed files to the digital preservation repository. #Development 
...
- Goobi Development to set at a project level the percentage of processes that will be required to go through Quality Assurance. So, for example, at the beginning of a large project the QA percentage could be set at `100%` as default causing all processes to have the Quality assurance step activated. Later this could be changed, say, to `50%` and this will trigger Goobi to randomly activate QA on 50% of processes. The percentage should be able to change at any point and then Goobi will refresh all processes where QA has not happened yet to the percentage level. #Development 
	...
	- #intranda ask for clarification on this
- Goobi Development to enable users to select which images from a complete item are needed by a customer. Image selection tool? #Development 
...
- Goobi Development to deliver to clients their ordered digitised content by offering a download link (similar to WeTransfer) The system will need to remind the requesting person to download their files and it must also record when the files have been downloaded to enable the closing of the process. #Development
...
1. Goobi will poll the EMu request tracker API at fixed intervals (TBC every 30 minutes during working hours and days) to pull back NEW requests with the status `Pending digitisation`. Each New request will be imported into Goobi with the following steps:
	- The request ticket will be imported as a sub project under the master project `Digitisation requests` #Development 
		...
		- #Leeds how will EMu communicate new requests?
	- The metadata from the requests will be imported as project properties (e.g.: specifications, information, email address of requesting person, Request tracker ticket number etc.) #Development 
		- #Geoff please see my comments above, as we need real data for testing
		- #Leeds please can you provide access to the requests module data via the API to #intranda 
		- #Leeds examples Any request with the markers `Digitisation` / `Download Request` which are also marked `Completed` are standard examples. See `RRREQ-2024-171` and `RRREQ-2024-116` as two recent examples.
		- #Leeds please clarify this
	...
	- Goobi will automatically assign a deadline (set centrally at, for example, 1 month) to the sub project #Development 
		...
		- #intranda include in quote (hopefully for batch)
	...
	- #Leeds decide how to proceed: use batches or sub projects or single processes
	- #Leeds decide how user assignment should be done
...
6. Workflow change: if `Proceed with digitisation` is recorded then the `Digitisation Triage` step will be set to `OPEN`. If `Delete process` is recorded then Goobi will delete the process entirely. #Development 
...
9. Image output settings: Here the user will record: 
	- the subset of pages/images that need to be digitised for the client (if appropriate) 
	...
	- If JPEGs are required by the customer
	- If PDF files are required by the customer
	- If TIFF files are required by the customer
...
20. Automatic file renaming: File rename plugin
...
26. #Variant If Moving image - Transcoding plugin using FFMPEG to create Mp4 with H264 codec #Development 
	...
	- #Leeds please can you arrange for some reference files to be transferred to intranda as requested?
...
30. #Variant if only some of the image files are required by the customer then they would be selected by the user at this stage using the newly developed image selection tool. #Development 
	- #done Geoff how does this look like? What is the user selecting in the order? How do we know if an item is complete?
	- #done Geoff-reply Earlier I described how the information in the request needs to be displayed as properties in the task page for all the processes as part of the sub project (request ticket). This will say, for example, that pages 3-10 are all that have been requested. So the user will look at the specification and will select the media files based on that in the image selection tool when it has been completed.
	- #intranda include in quote: extend image selection plugin to show property (page range)
1. #Variant If surrogate JPEGs or PDF files are required by the customer then Goobi will create the surrogates in a folder for delivery for only the images selected by the user for delivery to the client in the previous step.
	- #intranda #Leeds take this into account for the pricing proposal if not done already, PDF & JPEG?
	- #done Geoff we need clear definitions for the surrogates to be created
	- #done UOL Please can you define what surrogates you currently (or plan to) offer to clients.
	- #done Leeds Surrogates will be provided by the IIIF Services, Goobi does not need to create.
2. Goobi re-imports selected metadata fields from either the EMu API or the ALMA API into the METS file #Development 
	-  #intranda take this into account for the pricing proposal if not done already
3. Final QA: carried out by a different user than the main person assigned to the sub project and using the METS editor tool.
	-  #intranda take this into account for the pricing proposal if not done already
4. #Variant For moving images - Item received back from client: check box to select when item has been received. Date picker for received date. Quarantine time period (drop down list)
	- #intranda take this into account for the pricing proposal if not done already
5. #Variant For moving images - Quarantine: wait for x time period (set at the previous step)
...
6. #Variant For moving image - Pest Checking: Check box to record pest checking carried out. Sub process here for what to do if pests are found.
...
7. Item returned to storage: Check box for user to record that the item has been returned and a text box to enable the recording of the return location (Shelf)
8. Client Delivery: Goobi moves the files for client delivery (only those selected!) into a public folder and emails the client a link to download their files with the request tracker email in CC. (email to feature the request tracker number in the subject line). Goobi then monitors for when the files are downloaded and, when they are sends an email to the client thanking them for downloading and reminding them that the files will be available only for 7 additional days before they are deleted from the system. (email cc to request tracker as before). On the last day Goobi emails the client confirming that the files are no longer available. #Development 
	- #done Geoff would you prefer to use one of those existing tools instead of developing something from scratch?
	- #done Geoff-reply Absolutely if there is something we can use in the open source world then we should use it. the question is whether editing an existing tool is cheaper than developing a new one. The cheapest/easiest option is what we should go for.
9. Goobi creates the checksums for the digital files to be deposited and records that in the METS files *Note, this is to be confirmed*. #Development 
	- #done Geoff we are waiting for a confirmation here first.
	- #done Geoff-reply Yes, confirmation requested from UOL 
10. Goobi creates a digital object in the repository #Development 
	- #intranda take this into account for the pricing proposal if not done already
11. Goobi deposits the digital object in the repository #Development 
	- #intranda take this into account for the pricing proposal if not done already
12. Goobi receives back a message/confirmation that the deposit was successful *TBC* #Development 
	- #done Geoff we are waiting for a confirmation then
	- #Leeds confirmation requested
13. Goobi deletes the master images from the process and retains thumbnails for future reference. #Development 
	- #intranda take this into account for the pricing proposal if not done already
14. ~~Goobi closes the process and moves the deposited process to a new project entitled `Deposited Processes` Development~~
...
15. ~~Goobi changes the process template to a new process template called `Editing_Deposited_Processes` with the step: `Select editing action` set to OPEN.~~
...
- An excel export of the items to be digitised has been created with, as a minimum, a process title and the EMu IRN or the ALMA BIBNET reference. 
	...
	- #Leeds Please can you create a sample EMu and ALMA excel export file for intranda 
