...
This doesn’t make the Storage API itself dependent on METS – you can any object in it, with or without a METS file – but if you ask it to run (for example) a pipeline that _uses_ the storage service, that that pipeline stores PRONOM and virus scan outputs, it will store it in a METS file rather than as Fedora triples.
...
The remainder of this document uses the "core requirements identified by UK universities for digital preservation systems providers" mentioned in this slide:
