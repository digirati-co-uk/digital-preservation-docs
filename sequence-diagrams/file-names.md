...
Even files uploaded from a computer or collected from S3 may contain invalid paths, where "invalid" means not in our strictly limited _Preservation_ character set.

The UI has a "spreadsheet" (tabular) view that shows original and normalised (highlight where different).

It also allows you to edit original names, for example to restore characters that have been lost between source and here.
...
1. Our Preservation file-name-character set is more restrictive than most file systems, or S3. The Storage API only accepts normalised file names. Even on upload we may have to normalise. We preserve the original file name in METS `LABEL` attributes. And _maybe_ in DB rows in [files](../schema/files.sql.md).
