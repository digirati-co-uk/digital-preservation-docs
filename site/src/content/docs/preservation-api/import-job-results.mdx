...
The shared property `created` is the moment the API received the POST of the job. An ImportJobResult carries all four of `created`, `createdBy`, `lastModified` and `lastModifiedBy` — `lastModified` moves as the job progresses, which is a cheap way to tell a job that is still working from one that has stalled.
