...
`created`, `createdBy`, `lastModified` and `lastModifiedBy` are the [common resource properties](../overview#common-properties). `lastModifiedBy` is always overwritten with the identity the API resolved for you. `createdBy` is only filled in if you leave it out — one you supply is kept, and is what is stored and passed on to the Storage API.
