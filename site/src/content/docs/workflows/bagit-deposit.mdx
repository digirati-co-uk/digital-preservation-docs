...
<Aside type="caution">
Your payload has to end up under `data/objects/`, not directly under `data/`. Anything outside
`objects/` and `metadata/` is quietly ignored when files are added to the METS, and the diff then
refuses with a `422` naming a file it cannot find in the METS. If your bag's payload is not already
arranged that way, that is the one thing you do have to rearrange.
</Aside>
