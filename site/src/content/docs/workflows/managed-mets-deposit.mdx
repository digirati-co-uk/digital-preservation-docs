...
6. ### Add the files to the METS
   ...
   A list of paths relative to the root of the Deposit. Parent folders are created for you; folders are not expanded, so list every file you want. The `If-Match` is the Deposit's `metsETag` — **fetch the Deposit again to get it**. It is null in the response to creating one, and a pipeline run will have changed it since, so an ETag held from earlier in this walk-through is stale and will be refused with a `409`.
